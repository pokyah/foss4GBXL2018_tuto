...
data(BE_OSM_ADMIN)
bel = sf::st_as_sf(BE_OSM_ADMIN)
level4 = bel %>% filter(tag.admin.level == 4)
wallonia = level4 %>% filter(tag.name.fr == "Wallonie")
sf::st_crs(wallonia)
# croping (masking)
...
# get admin boundaries of Wallonia with https://github.com/pokyah/BelgiumMaps.Admin
library(BelgiumMaps.Admin)
data(BE_OSM_ADMIN)
bel = sf::st_as_sf(BE_OSM_ADMIN)
level4 = bel %>% filter(tag.admin.level == 4)
